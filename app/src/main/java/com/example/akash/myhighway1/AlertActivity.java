...
public class AlertActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        db = new DatabaseHandler(AlertActivity.this);
        ...
        SendAlert.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
               // hideKeyboard();
                ...
                if (!AlertType.equals("") && totalfriends > 0 && !myNumber.equals("") && isNetworkAvailable(AlertActivity.this)) {
                    String AlertDesc = "," + AlertType + (AlertMessage.getText().toString().equals("") ? ",NA" : ("," + AlertMessage.getText().toString()));
                    User user = new User(AlertActivity.this);
                    String lastLoc = user.getLastLocationLat() + ":" + user.getLastLocationLong();
                    AlertDesc += "," + myNumber;
                    AlertDesc += "," + lastLoc;
                    String s1 = contactlist + AlertDesc;
                    // Toast.makeText(AlertActivity.this, s1, Toast.LENGTH_SHORT).show();
                    String s[] = {getString(R.string.appwebsite) + "/api/sendAlert.php", "alert", s1};
                    sendRequest sendrequest = new sendRequest();
                    sendrequest.execute(s);
                }
                else if(AlertType.equals("")){
                    Toast.makeText(AlertActivity.this, "First Select Type of AlertActivity", Toast.LENGTH_SHORT).show();
                    SendAlert.setEnabled(true);
                }
                else if (myNumber.equals("")) {
                    Toast.makeText(AlertActivity.this, "Make  Sure You have registered Your Number", Toast.LENGTH_LONG).show();
                    SendAlert.setEnabled(true);
                } else if (!isNetworkAvailable(AlertActivity.this)) {
                    Toast.makeText(AlertActivity.this, "Make Sure Internet Is working!!!", Toast.LENGTH_SHORT).show();
                    SendAlert.setEnabled(true);
                }
        ...
        getSupportActionBar().setTitle("AlertActivity");
                ...
                public void onResponse(String response) {
                    ...
                    try {
                        ...
                        if (jsonObject.getInt("success") == 1) {
                            ...
                            editor.putString("AlertActivity", AlertType).commit();
                            startActivity(new Intent(AlertActivity.this, ChatActivity.class));
                            finish();
                        }
                        else if (jsonObject.getInt("success")==0){
                            SendAlert.setEnabled(true);
                            Toast.makeText(getApplicationContext(), "Because of some reason we aren't able to send AlertActivity", Toast.LENGTH_LONG).show();
                        }
                            ...
                            SendAlert.setEnabled(true);
                            Toast.makeText(AlertActivity.this, "You may no have any sshs user as Friend", Toast.LENGTH_LONG);
                        }
